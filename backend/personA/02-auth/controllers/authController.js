...
exports.resetPassword = async (req, res, next) => {
  try {
    ...
    if (phone) {
      ...
      if (decoded.type !== 'phone_otp_verified' || decoded.purpose !== OTP_PURPOSES.RESET_PASSWORD) {
        return res.status(400).json({
          success: false,
          message: 'Invalid phone verification token for password reset',
        });
      }
    }

    const user = await findAccountByQuery(
      email ? { email: email.trim().toLowerCase() } : { phoneNumber: phone }
    );
...
exports.createNewPassword = async (req, res, next) => {
  try {
    const { phoneVerificationToken, newPassword } = req.body;

    if (!phoneVerificationToken || !newPassword) {
      return res.status(400).json({
        success: false,
        message: 'phoneVerificationToken and newPassword are required',
      });
    }
    ...
    if (decoded.type !== 'phone_otp_verified') {
      return res.status(400).json({
        success: false,
    ...
    const user = await findAccountByPhone(decoded.phoneNumber);
